...
MeasurementValidator::MeasurementValidator(
    MeasurementValidatorConfig config)
    : config_{config}
{
    ...
    std::optional<double> measured_value)
{
    ...
    {
        return {
            .status = MeasurementStatus::NonFinite,
            .measured_value = measured_value,
        };
    }

    if(value < config_.minimum_value ||
       value > config_.maximum_value)
    {
        return {
...
void validate_config(
    const MeasurementValidatorConfig& config)
{
    ...
    {
        throw std::invalid_argument(
            "maximum rate per second must be finite "
            "and non-negative");
    }
}
