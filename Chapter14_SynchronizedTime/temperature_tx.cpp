...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Chronos temperature simulator -> "
              << destinationIp << ':' << port
              << " every " << periodMs << " ms\n";
    ...
    while (!stopRequested)
    {
        ...
        // Deliberately simple synthetic temperature data: a slow sawtooth.
        const double temperature =
            20.0 + static_cast<double>(sequence % 40u) * 0.05;

        const ch14::Temperature measurement{
            .sequence = sequence,
        ...
        ch14::writeTemperature(payloadWriter, measurement);
        if (!payloadWriter.ok())
        {
            std::cerr << "Failed to encode temperature payload\n";
            continue;
        }
        ...
        std::cout << "TX  seq=" << std::setw(5) << measurement.sequence
                  ...
                  << std::setw(5) << measurement.degreesCelsius << " C"
                  << "  t=";
